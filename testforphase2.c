...
// Condition variable for hybrid assist thread to wait on ECU signals
pthread_cond_t hybridAssistConditional;
...
// Fuel Subsystem
// Uses a condition variable to pause fuel consumption when engine is OFF
...
void *ecu_thread(void *arg)
{
  ...
  while (1)
  {
    ...
    // signal hybrid assist thread that ECU has updated the allowed state
    // so it can re-evaluate assist logic without busy-waiting
    pthread_mutex_lock(&hybridAssistLock);
    pthread_cond_signal(&hybridAssistConditional);
...
// Uses a condition variable to wait for ECU signals before applying assist logic,
// avoiding busy-waiting as required by Phase II synchronization requirements.
void *hybrid_assist_thread(void *arg)
{
  ...
  {
    //================
    // CRITICAL SECTION - wait for ECU signal before evaluating assist logic
    //================
    pthread_mutex_lock(&hybridAssistLock);

    // wait for ECU to signal that conditions may have changed
    pthread_cond_wait(&hybridAssistConditional, &hybridAssistLock);
    ...
    //================
    // CRITICAL SECTION - protect battery_level, hybrid state, and charging state
    //================
    pthread_mutex_lock(&hybridAssistLock);

    // only run assist logic if ECU says we are allowed to use electric assist
    if (electric_assist_allowed == 0)
    {
      // ECU has disabled assist — turn everything off and exit critical section
      electric_assist_state = 0;
      charging_state = 0;
      hybrid_mode = 0;
      pthread_mutex_unlock(&hybridAssistLock);

      // check if mode changed and log event
      if (hybrid_mode != previous_mode)
      {
        hybridAssistChange = 2; // turned off
        struct Event assist_event;
        strncpy(assist_event.title, "HYBRID OFF", sizeof(assist_event.title));
        snprintf(assist_event.description, sizeof(assist_event.description),
                 "Hybrid assist deactivated (battery low or engine off)");
        assist_event.timestamp = time_elapsed_total;
        enqueue_event(&assist_event);
        previous_mode = hybrid_mode;
      }
      continue;
    }

    // check if speed is decreasing — regeneration only occurs on deceleration
    // and only when engine is ON, as required by Phase II rules
    if (direction == -1 && engine_state == 1)
    {
      // REGENERATION — charging must strictly only occur when speed is decreasing and engine is on
      hybrid_mode = 3;
      electric_assist_state = 0;
      charging_state = 1;
      battery_level += 0.07;
    ...
    { // handle all cases for hybrid assist during acceleration
      if (speed > 0 && speed <= 30)
    ...
    // check hybrid mode for ECU event logging
    if (hybrid_mode != previous_mode)
    {
      if (previous_mode == 0 && hybrid_mode != 0)
      {
        hybridAssistChange = 1; // turned on
        struct Event assist_event;
        strncpy(assist_event.title, "HYBRID ON", sizeof(assist_event.title));
        snprintf(assist_event.description, sizeof(assist_event.description),
                 "Hybrid assist activated, mode: %d", hybrid_mode);
        assist_event.timestamp = time_elapsed_total;
        enqueue_event(&assist_event);
      }
      else if (hybrid_mode == 0 && previous_mode != 0)
      {
        hybridAssistChange = 2; // turned off
        struct Event assist_event;
        strncpy(assist_event.title, "HYBRID OFF", sizeof(assist_event.title));
        snprintf(assist_event.description, sizeof(assist_event.description),
                 "Hybrid assist deactivated");
        assist_event.timestamp = time_elapsed_total;
        enqueue_event(&assist_event);
      }
      previous_mode = hybrid_mode;
    ...
    // check if battery level just crossed the low battery warning threshold
    // log event once when it drops below 20%
    if (battery_level < 20 && last_battery_amount_logged >= 20)
    {
      struct Event batt_event;
      strncpy(batt_event.title, "LOW BATTERY", sizeof(batt_event.title));
      snprintf(batt_event.description, sizeof(batt_event.description),
               "Battery dropped below 20%%: %.2f%%", battery_level);
      batt_event.timestamp = time_elapsed_total;
      enqueue_event(&batt_event);
    }
...
// subroutine to handle command-line argument initialization
// NOTE: This section is temporary for Phase II only and will be removed in Phase III
// It is isolated here in its own function so it can be cleanly removed later
...
int main(int argc, char *argv[])
{
  ...
  // --- Phase II: Initialize from command-line arguments ---
  // This call is temporary and will be removed in Phase III
  init_from_args(argc, argv);

  // initialize remaining state that is not set via command line
  rpm_zone = 0;
  ...
  pthread_cond_init(&hybridAssistConditional, NULL);
